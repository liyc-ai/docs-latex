...
\begin{frame}[allowframebreaks,fragile]{Preliminaries}
	\begin{itemize}
		...
		\item By selecting a proper regularizer on the learned reward function $r$, \citep{gail} associated the maximum entropy IRL problem with GAN, thus getting GAIL's objective:
		      \[
...
\begin{frame}[allowframebreaks,fragile]{Method}
	...
	\begin{proof}
		...
		\end{enumerate}
	\end{proof}
	From Theorem \ref{thm:asaf}, we find that optimizing the inner $\tilde{\pi}$ will give us the optimal outer $\pi_G$. We thus get ASAF, a new imitation learning algorithm with pseudocode shown in Algorightm \ref{algo:asaf}.

	\begin{algorithm}[H]
		\caption{ASAF}
		\label{algo:asaf}
		\begin{algorithmic}
			\REQUIRE expert trajecotries $\mathcal{D}_E = \{\tau_i^{(E)}\}_{i=1}^{N_E}$
...
\begin{frame}{Discussion}
	\begin{itemize}
		...
		\]
		\item Reduction to transition-wise scenario: The author also discussed a novel transion-wise objective (connected to \citep{DBLP:journals/corr/abs-1710-11248}), which is only suit for discrete action space.
		% \begin{itemize}
		...
		% 	\item The author also discussed a novel transion-wise objective (connected to \citep{DBLP:journals/corr/abs-1710-11248}), which is suit for discrete action space.
		% \end{itemize}
